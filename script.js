...
let allFlags = allFlagsData.flags;

// تهيئة الموقع
window.addEventListener('DOMContentLoaded', function() {
    displayFlags(allFlags);
    updateCount(allFlags.length);
    setupEventListeners();
});

function setupEventListeners() {
    document.getElementById('search').addEventListener('input', filterFlags);
    document.getElementById('filter').addEventListener('change', filterFlags);
    document.getElementById('difficulty').addEventListener('change', filterFlags);
}

// عرض الأعلام (معدّلة للانتقال لصفحة منفصلة)
function displayFlags(flags) {
    ...
    flags.forEach(flag => {
        ...
        // الانتقال لصفحة العلم عند الضغط
        card.addEventListener('click', () => {
            window.location.href = `flag.html?id=${flag.id}`;
        });
        
        card.innerHTML = `
            <img src="https://flagcdn.com/w320/${flag.id}.png" alt="${flag.name}">
            <h3>${flag.name}</h3>
            <span class="badge ${flag.difficulty}">${flag.difficulty}</span>
        `;
...
function filterFlags() {
    ...
    if (difficulty !== 'all') {
        filtered = filtered.filter(flag => flag.difficulty === difficulty);
    }
...
function updateCount(count) {
    document.getElementById('count').textContent = `${count} علم متاح`;
}